...
import game.rise_of_valor.game_engine.Camera;
...
public class Player extends Character {
    ...
    public void draw(GraphicsContext gc, Camera camera) {
        ...
        if (currentSprite < sprites.size()) {
            ...
            gc.fillOval(inertiaPositionX + shadowWidth + shadowXOffset - camera.getX(), inertiaPositionY + scaledSize - 30 - camera.getY(), shadowWidth, 10);
            System.out.println("cameraX:"+camera.getX()+", cameraY:"+camera.getY());
            if (facingLeft) {
                gc.save();
                gc.scale(-1, 1);
                gc.drawImage(sprite, -inertiaPositionX - scaledSize + camera.getX(), inertiaPositionY - camera.getY(), scaledSize, scaledSize);
                gc.restore();
            } else {
                gc.drawImage(sprite, 0, 0, width, height, inertiaPositionX - camera.getX(), inertiaPositionY - camera.getY(), scaledSize, scaledSize);
            }
        } else {
